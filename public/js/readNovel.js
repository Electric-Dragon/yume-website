...
let getComments = async () => {
    ...
    data.forEach(comment => {
        ...
        let element = ` <div class="flex">
                            ...
                            <img class="mt-2 rounded-full w-8 h-8 sm:w-10 sm:h-10" src="${user.pfp}" alt="">
                            </div>
                            <div class="flex-1 border rounded-lg px-4 py-2 sm:px-6 sm:py-4 leading-relaxed">
                            <strong>${user.username}</strong> <span class="text-xs text-gray-400">${date.getDate()}/${date.getMonth()+1}/${date.getFullYear()}</span>
                            <p class="text-sm">
                               ${content}
                            </p>
